	...
	def train(self, config):
		...
		if args.data_name != 'amazon':
			...
			idx_train, idx_rest, y_train, y_rest = train_test_split(index, labels[3305:], stratify=labels[3305:],
																			train_size=0.4, random_state=args.seed, shuffle=True)
			idx_valid, idx_test, y_valid, y_test = train_test_split(idx_rest, y_rest, stratify=y_rest,
																			test_size=0.67, random_state=args.seed, shuffle=True)
			print(idx_train[:10])
		...
		if args.model == 'CARE':
			if args.data_name=='KDK':
				...
				inter1 = InterAgg5(features, feat_data.shape[1], args.emb_size, adj_lists, [intra1, intra2, intra3, intra4, intra5], inter=args.inter, step_size=args.step_size, cuda=args.cuda)
			elif args.data_name == "yelp" or args.data_name == "amazon":
				intra1 = IntraAgg(features, feat_data.shape[1], cuda=args.cuda)
				intra2 = IntraAgg(features, feat_data.shape[1], cuda=args.cuda)
				intra3 = IntraAgg(features, feat_data.shape[1], cuda=args.cuda)
				inter1 = InterAgg3(features, feat_data.shape[1], args.emb_size, adj_lists, [intra1, intra2, intra3], inter=args.inter, step_size=args.step_size, cuda=args.cuda)
			else:
				intra1 = IntraAgg(features, feat_data.shape[1], cuda=args.cuda)
				inter1 = InterAgg1(features, feat_data.shape[1], args.emb_size, adj_lists, [intra1], inter=args.inter, step_size=args.step_size, cuda=args.cuda)
		...
		dir_saver = os.path.join("/temp/CARE-GNN_models/", self.ckp.log_file_name)
		...
		for epoch in range(args.num_epochs):
			...
			if (epoch+1) % args.valid_epochs == 0:
				...
				# CARE-GNN을 학습할 경우의 test!
					...
					print("Valid at epoch {}".format(epoch))
					gnn_auc_val, gnn_recall_val, gnn_f1_val = test_care(idx_test, y_test, gnn_model, args.batch_size, self.ckp, flag="val")
					if gnn_auc_val > auc_best:
						gnn_recall_best, f1_mac_best, auc_best, ep_best = gnn_recall_val, gnn_f1_val, gnn_auc_val, epoch
						if not os.path.exists(dir_saver):
							os.makedirs(dir_saver)
		...
		if args.model == 'SAGE':
			...
			gnn_auc, gnn_recall, gnn_f1 = test_care(idx_test, y_test, gnn_model, args.batch_size, self.ckp, flag="test")
